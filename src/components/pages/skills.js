import React from "react";

const Skills = () => {
  return (
    <div className="jumbotron">
      <div style={{ alignItems: 'center' }}>
        <h1 style={{ fontSize: '60px', marginTop: '5%' }}>Skills</h1>
        <div>
          <p
            style={{
              width: "75%",
              height: "400px",
              overflowY: 'auto',
              padding: '20px', 
              backgroundColor: 'rgba(0, 0, 0, 0.7)', 
              border: '2px solid rgba(255, 255, 255, 0.3)', 
              borderRadius: '8px', 
              boxShadow: '0 4px 10px rgba(0, 0, 0, 0.5)', 
            }}
          >
            <span style={{ color: 'red', fontWeight: "900" }}>Certifications: </span>I have recently completed the Front-End Web Developer Skills Bootcamp Professional Certificate intensive program from edX.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>HTML5 /CSS: </span>Proficient in creating well-structured and visually appealing web pages using HTML and CSS.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>JavaScript:</span> Proficient in JavaScript, including framework - <span style={{ color: 'blue' }}>React </span> to create dynamic and interactive web applications. Familiar with component-based architecture and virtual DOM in React.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>Front End Frameworks and libraries:</span> Experienced in using front-end frameworks such as React, Bootstrap, and jQuery. <br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>Node.js:</span> I am skilled in using Node.js to develop server-side applications and npm (Node Package Manager) to easily install, manage, and share code packages.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>APIs:</span> I have knowledge of how to use APIs to integrate different data, applications, and services.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>Responsive Design:</span> Skilled in creating responsive designs that work across different devices and screen sizes to ensure a seamless user experience.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>User Experience (UX):</span> Understanding of user experience principles and skilled in designing websites that are easy to use and navigate, providing users with an enjoyable experience.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>Accessibility:</span> Knowledgeable in accessibility guidelines such as WCAG 2.1 and skilled in creating websites that are accessible to everyone, including those with disabilities, to ensure inclusivity.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>Cross-Browser Compatibility:</span> Skilled in writing code that works across different browsers and devices to ensure maximum accessibility.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>Performance Optimization: </span>Skilled in optimizing website performance, including skills such as minification, image optimization, and lazy loading, to create fast-loading websites that provide a smooth user experience.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>Version Control: </span>Proficient in Github and version control, allowing for collaboration with other developers and contributions to larger projects.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>Code Editors: </span>Proficient in using Visual Studio Code, a powerful code editor with features such as code highlighting, code completion, and debugging tools.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>Debugging and Troubleshooting: </span>Jest / Enzyme - Skilled in troubleshooting and debugging code, demonstrating strong problem-solving abilities.<br /><br />
            <span style={{ color: 'orange', fontWeight: "700" }}>Communication: </span>Experience with Slack and Zoom, strong communication skills, with the ability to articulate technical concepts to both technical and non-technical stakeholders.<br /><br />
            <span style={{ color: 'red', fontWeight: "900" }}>Skills I want to learn: </span>Interested in learning back-end development using technologies and programming languages such as SQL, Python, C++ and mobile app development using React Native.<br /><br />
          </p>
        </div>
        
        <ul id="icn" style={{ alignItems: 'center', paddingBottom: "3%", paddingTop: "3%", marginLeft: "5%" }}>
          <i style={{ fontSize: "60px", color: "yellow" }} className="devicon-javascript-plain"></i>
          <i style={{ fontSize: "60px", color: "red" }} className="devicon-html5-plain"></i>
          <i style={{ fontSize: "60px", color: "blue" }} className="devicon-css3-plain-wordmark colored"></i>
          <i style={{ fontSize: "60px", color: "#ADD8E6" }} className="devicon-react-original"></i>
          <i style={{ fontSize: "60px", color: "red" }} className="devicon-npm-original-wordmark colored"></i>
          <i style={{ fontSize: "60px", color: "green" }} className="devicon-nodejs-plain colored"></i>
          <i style={{ fontSize: "60px", color: "white" }} className="devicon-github-original-wordmark"></i>
          <i style={{ fontSize: "60px", color: "orange" }} className="devicon-jquery-plain"></i>
        </ul>

      </div>
    </div>
  );
};

export default Skills;
